...
//        }
    ...
    public boolean contextMatters(int iContext, Dataset dataset, int minimumRating, float significant) {
        ...
        //System.out.println(countAffect);
        this.warShall(iContext, samples.size(), a);
//        this.generateHierachyFromGraph(graph);
...
//            System.out.println("No Matter");
    ...
    public float computePairTTest(List<Double> lstSample1, List<Double> lstSample2) {
        ...
        try {
            ...
            return Math.abs((float) TestUtils.pairedTTest(sample1, sample2));
        } catch (Exception ex) {
            return 2;
        }
    }
...
//        List<List<List<Integer>>> i = new ArrayList<List<List<Integer>>>();
    ...
    public void runGFSG(Dataset dataset, int minimumRating, float significant) {
        ...
        for (int iContext = 0; iContext < numContextDims; iContext++) {
            this.contextMatters(iContext, dataset, minimumRating, significant);
        }
    }
...
//        }
    ...
    public void generateContextCombinationFromHierachyForStudyAdvisor() {
        numAttrAtContextHierachy = (new int[hierachy.size()]);
        int count = 1;
        for (int i = 0; i < hierachy.size(); i++) {
            numAttrAtContextHierachy[i] = hierachy.get(i).size();
        ...
        int numCombination = hierachy.get(0).size();
        contextsGereration = new String[numCombination];
        for (int i = 0; i < numCombination; i++) {
            contextsGereration[i] = Integer.toString(i)+"::";
