 ...
 * @author Hiep
 */
public class STI extends Algorithm {

    public int _minRating = 1;
                        ...
                        for (Map.Entry<String, Float> entry : _dataset.mRatingWithContext[u][i].contextScorePairs.entrySet()) {
                            if (entry.getKey().equals(profile.context)) {
                                if (profile.ratings == null) {
                                    ...
                                    for (int z = 0; z < _dataset.numUser; z++) {
                                        profile.ratings[z] = -1;
                                    }
                                }
...
//                    }
                    ...
                    String t = profile.itemID + "=>" + profile.context;
                    if (itemContextCombination.contains(t)) {
                        profile.init(_dataset.numUser);
                        lstData.add(profile);
                    }
                }
            }
...
//        System.out.print(mf.prediction(u, newID) + "---" + biasContext.get(u + "=>" + c).floatValue() + "   ");
        if (predictiveValue < _minRating) {
            return _minRating;
        }
        if (predictiveValue < _maxRating) {
    ...
    public static void main(String[] arg) {
        ...
        dm.dataProcessor("C:\\Users\\Hiep\\Dropbox\\Recommender_Systems\\dataset_full\\hmusic.l3", 3);
        m = 5;
        Algorithm.N = 10;
        for (int i = 0; i < m; i++) {
